...
import { makeTask, makeAgent, makeComment } from "../../helpers/factories.js"
...
describe("updateTask / valid transitions", () => {
  ...
  for (const [from, to] of valid) {
    test(`${from} → ${to}: status changes and hook fires`, async () => {
      ...
      const task = await seedTask({ status: from, in_progress_since: from === 'in_progress' ? new Date() : undefined })
      ...
      if (from === 'need_info') {
        const repliedComment = makeComment({ kind: 'need_info', reply: 'answered', content: 'question' })
...
describe("updateTask / need_info reply cycle", () => {
  test("update with matching comment id and non-empty reply → reply populated, task stays need_info", async () => {
    ...
    const replyComment = makeComment({ id: "c-1", kind: 'need_info', reply: "the answer", content: "what?" })
  ...
  test("reply on kind: regular comment → validation_error", async () => {
    ...
    const task = await seedTask({ status: 'in_progress', in_progress_since: new Date(), comments: [regularComment] })
