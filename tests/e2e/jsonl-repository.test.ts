...
const runFail = <A, E>(effect: Effect.Effect<A, E>): Promise<{ _tag: string; [k: string]: unknown }> =>
  Effect.runPromise(
    effect.pipe(
      Effect.matchEffect({
...
describe("JsonlTaskRepository / e2e", () => {
  ...
  test("update() overwrites the correct line; other lines unchanged", async () => {
    ...
    await run(repo.save(t1))
    await run(repo.save(t2))
    const updated = { ...t1, title: "Updated Title", status: 'in_progress' as const, in_progress_since: new Date() }
